...
erpnext.taxes_and_totals = class TaxesAndTotals extends erpnext.payments {
	...
	apply_pricing_rule_on_item(item) {
		...
		// Calcul des valeurs
		const PU = item.rate_with_margin;
		const SD = flt(item.custom_supplementary_discount_amount) || 0;
		const RM = flt(item.discount_percentage) / 100 || 0;
		const EET = flt(item.custom_energy_efficiency_tax) / 100 || 0;
		const RTA = flt(item.custom_automotive_technical_review) || 0;

		// Calcul du prix initial après la remise supplémentaire
		const initialPrice = PU - SD;
	
		// Calcul du montant de la remise
		const discountAmount = initialPrice * RM;

		// Calcul du montant de la EET
		const EETPrice= PU * EET ;
		
		// Calcul du prix final
		const finalPrice = (initialPrice - discountAmount );
	
		// Mise à jour de item_rate
		item.rate = finalPrice;

		//Initialisation du Remise % de base
		const discount_percentage = RM;
		// Calcul du montant de la remise totale
		const totalDiscountAmount = discountAmount + SD;

		// Calcul du pourcentage de remise par rapport au PU initial
		const totalDiscountPercentage = (totalDiscountAmount / initialPrice) * 100;
		
		// Calcul du pourcentage de remise par rapport au PU initial
		const supplementaryDiscountPercentage = (SD / initialPrice) * 100;
		

		if (item.discount_percentage || item.custom_supplementary_discount_amount) {
			item.discount_amount = flt(totalDiscountAmount);
		}
		
		if(item.custom_energy_efficiency_tax){
			console.log('item.custom_energy_efficiency_tax',item.custom_energy_efficiency_tax)
			item.rate += EETPrice;
			console.log('EETPrice',EETPrice);
		}

		if(item.custom_automotive_technical_review){
			console.log('item.custom_automotive_technical_review',item.custom_automotive_technical_review)
			item.rate += RTA;
			console.log('RTA',RTA);
		}

		// Mise à jour de la valeur du modèle
		frappe.model.set_value(item.doctype, item.name, "rate", item.rate);
	}
