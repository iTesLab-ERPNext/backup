...
def item_query(doctype, txt, searchfield, start, page_len, filters, as_dict=False):
	...
	warehouse = filters.pop("set_warehouse", None)
	...
	if warehouse:
		...
		"""
		opening_stock_col = "IFNULL(tabBin.projected_qty, 0) AS opening_stock"
	else:
		bin_join = ""
		opening_stock_col = "NULL AS opening_stock"

	frappe.log_error(
		f"Warehouse: {warehouse}",
		"Item Query Warehouse"
	)
	...
	return frappe.db.sql(
		...
		""".format(
			...
			bin_join=bin_join,
			opening_stock_col=opening_stock_col
		),
		{
			"today": nowdate(),
			"txt": "%%%s%%" % txt,
